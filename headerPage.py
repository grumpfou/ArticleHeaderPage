...
class HeaderPage:
	def __init__(self,filepath,title=None,short=None,author=None,replace=False,
		...
		# if short==None:
			# pattern = '^.*20[0-9]{2}(_?)[0-9]{2}(_?)[0-9]{2}(_?)'
			# s = os.path.split(filepath)[1]
			# sch = re.match(pattern,s)
			# if sch:
				# short = sch.group(0)
				# short = short.replace('_','')
				# i = short.find('20')
				# short = short[:i]+'_'+short[i:]
				# print 'Find a short title :', short
...
class Dialog (QtGui.QDialog):
	def __init__(self,*args,**kargs):
		...
		self.bibtex_prase_button = QtGui.QPushButton("Prase")
		self.short_edit = QtGui.QLineEdit()
		self.authors_edit = QtGui.QLineEdit()
		self.title_edit = QtGui.QLineEdit()
		self.file_edit = QtGui.QLineEdit()
		self.notes_edit = QtGui.QTextEdit()

		self.replace = QtGui.QSpinBox()
	...
	def prase(self):
		...
		def prase_wid():
			...
			if len(a.entries)==0:
				QtGui.QMessageBox.critical(self,"Wrong Prasing","Wrong Bibtex reference.")
				return False
			d = a.entries[0]
			if not d.has_key('file'):
				QtGui.QMessageBox.critical(self,"Wrong Prasing","Bibtex do not contain the file.")
				return False
			if not (d.has_key('year') or d.has_key('date') ):
				QtGui.QMessageBox.critical(self,"Wrong Prasing","Bibtex do not contain the year or the date.")
				return False
			if not d.has_key('author'):
				QtGui.QMessageBox.critical(self,"Wrong Prasing","Bibtex do not contain the author.")
				return False
			if not d.has_key('title'):
				QtGui.QMessageBox.critical(self,"Wrong Prasing","Bibtex do not contain the title.")
				return False
			...
			if d.has_key('year'): y = d['year']
				...
				if len(tmp)==0:
					QtGui.QMessageBox.critical(self,"Wrong Prasing",
			...
			# if len(authors)>2:
				# short = authors[0][0]+ ' et al. ('+y+')'
			# else:
				# short = ' and '.join([a[0] for a in authors])+' ('+ y+')'
			title = d['title']
			title = title.replace('{','')
			title = title.replace('}','')
			...
			if len(files)==0:
				...
				item = QtGui.QInputDialog.getItem(textedit,
			...
			# m = re.search('PDF:.*\.pdf',file,re.IGNORECASE)
			# if m==None:
			# 	QtGui.QMessageBox.critical(self,"Wrong Prasing",
			# 							"No pdf file detected in bibtex file.")
			# 	return False
		...
		wid =  QtGui.QDialog(self,QtCore.Qt.Tool)
		but = QtGui.QPushButton("Prase")
		lay = QtGui.QVBoxLayout()
		lay.addWidget(textedit)
		lay.addWidget(but)
		wid.setLayout(lay)
		self.connect(but, QtCore.SIGNAL("clicked()"), prase_wid)
		self.connect(QtGui.QShortcut(QtGui.QKeySequence("Ctrl+Return"), self), QtCore.SIGNAL('activated()'), prase_wid)
		wid.show()
	...
	def generate(self):
		...
		r = QtGui.QMessageBox.question(self,"Confimation",m,
		...
		if r == QtGui.QMessageBox.Yes:
			...
			try:
				...
				QtGui.QMessageBox.critical(self,'HeaderPage','Error when creating '+\
					'the file: \n'+str(mess))
				raise e
			else:
				QtGui.QMessageBox.information(self,'HeaderPage','Header page '+\
		...
		# print 'short : ',short.encode('utf-8')
